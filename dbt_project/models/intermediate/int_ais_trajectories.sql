...
{{
  config(
    ...
    pre_hook="{% if is_incremental() %} DELETE FROM {{ this }} WHERE date_partition = '{{ var('target_date', '2024-01-01') }}' {% endif %}"
  )
}}
...
WITH staging AS (
    ...
    -- Optim: Cap upper bound to avoid full scan if table is huge
...
enriched AS (
    SELECT 
        *,
        ...
        {{ dbt_utils.generate_surrogate_key(['mmsi', 'timestamp', 'lat', 'lon']) }} as id,
        
        -- Calculated Speed (Knots) => (Meters / Seconds) * 1.94384
        CASE 
            WHEN delta_time_sec > 0 THEN (delta_dist_meters / delta_time_sec) * 1.94384 
            ELSE 0 
